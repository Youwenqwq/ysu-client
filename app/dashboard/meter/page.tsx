    ...
    try {
      ...
      const message =
        e instanceof MeterError && e.message === "NOT_BOUND"
          ? t("meter.notBound")
          : t("meter.loadFailed", { message: e instanceof Error ? e.message : String(e) });
      // 已有数据时刷新失败不顶掉旧数据
  ...
  return (
    <div className="flex flex-col gap-4">
          ...
          <Card key={`${meter.deviceName}-${mi}`}>
            <CardContent className="flex flex-col gap-4">
              ...
              <div className="flex items-end gap-2">
                <span className="text-5xl font-bold leading-none tabular-nums tracking-tight">
                  {meter.remaining}
                </span>
                ...
                <span className="ml-auto pb-0.5 text-sm text-muted-foreground">
                  {t("meter.balanceApprox", { amount: (meter.remaining * meter.price).toFixed(2) })}
                  {" · "}
      ...
      <Card>
        ...
        <CardContent>
          {daily.length === 0 ? (
            <p className="py-4 text-center text-sm text-muted-foreground">{t("meter.trendEmpty")}</p>
          ) : (
            <div className="flex flex-col gap-1.5">
              <div className="flex h-28 items-end gap-[3px]">
                {daily.map((d) => (
                  <div
                    key={d.date}
                    title={t("meter.dayUseTooltip", { date: d.date.slice(5), use: String(d.use) })}
                    className={cn(
                      "min-w-1 flex-1 rounded-sm transition-colors",
                      d.date === today ? "bg-primary" : "bg-primary/25 hover:bg-primary/50",
                    )}
                    style={{ height: `${maxDaily > 0 ? Math.max(3, (d.use / maxDaily) * 100) : 3}%` }}
                  />
                ))}
              </div>
                ...
                <div key={`${r.time}-${i}`}>
                  ...
                  <div className="flex items-center gap-3 py-3">
                    ...
                    <div className="ml-auto flex shrink-0 items-baseline gap-2">
                      ...
                      {r.fare > 0 ? (
                        <span className="text-xs text-muted-foreground tabular-nums">¥{r.fare}</span>
                      ) : null}
                    </div>
                  </div>
                </div>
