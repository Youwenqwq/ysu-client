...
async function fetchActiveRoleId(): Promise<string | null> {
  const result = await rawPost(`${baseUrl()}/xsfw/sys/swpubapp/indexmenu/getAppConfig.do`, {
  ...
  const roleId =
    active !== null && typeof active === "object" ? (active as Record<string, unknown>)["id"] : null
  if (!roleId) {
...
async function activateRole(roleId: string): Promise<void> {
  const setResult = await rawPost(`${baseUrl()}/xsfw/sys/swpubapp/userinfo/setXgCommonAppRole.do`, {
...
async function resolveTerm(year?: string, term?: string): Promise<{ cpxn: string; cpxq: string }> {
  ...
  throw new XgxtProtocolError(`未找到匹配的测评学年学期: year=${year ?? ""} term=${term ?? ""}`)
}
...
export async function queryEvaluationTerms(): Promise<EvaluationTerm[]> {
  return runWithReauth(async () => {
    ...
    const rows =
      data !== null && typeof data === "object" ? (data as Record<string, unknown>)["XNXX"] : null
    if (!Array.isArray(rows)) {
...
export async function queryEvaluationResult(
  ...
  return runWithReauth(async () => {
    ...
    const data = await postController(API_PATHS.cjByXn, {
      CPXN: cpxn,
      CPXQ: cpxq,
    })
...
export async function queryEvaluationRadar(
  ...
  return runWithReauth(async () => {
    const { cpxn, cpxq } = await resolveTerm(year, term)
    const data = await postController(API_PATHS.redar, {
      CPXN: cpxn,
      CPXQ: cpxq,
    })
