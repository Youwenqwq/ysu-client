...
function getChannelCandidates(data: VersionManifest, channel: UpdateChannel): UpdateCandidate[] {
  const topLevel = toTopLevelEntry(data)
  const stable = data.channels?.stable ? { ...topLevel, ...data.channels.stable } : topLevel
  const entries = channel === "stable" ? [stable] : [stable, data.channels?.prerelease]
...
function getHighestVersionCandidate(candidates: UpdateCandidate[]): UpdateCandidate | undefined {
  const sorted = [...candidates].sort((a, b) => compare(a.normalizedVersion, b.normalizedVersion))
...
async function getApkUpdateCandidate(
  ...
  const sorted = candidates
    .filter(
      (candidate) =>
        typeof candidate.apkVersionCode === "number" && candidate.apkVersionCode > installedBuild
    )
...
export async function checkForUpdate(
  ...
  try {
    ...
    const webUpdateCandidate = getHighestVersionCandidate(
      candidates.filter((candidate) => isNewer(APP_VERSION, candidate.webVersion, channel))
    )
    ...
    const result: UpdateInfo = {
      ...
      downloadUrl: getWebDownloadUrl(mirrorPrefix, displayCandidate.webDownloadUrl, distZipUrl),
