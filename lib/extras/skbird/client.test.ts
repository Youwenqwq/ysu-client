...
describe("skbirdImageUrl", () => {
  it("相对路径补前导斜杠（img_paths 实测为 upload/xx.jpg 形态）", () => {
    ...
    expect(skbirdImageUrl("upload/a/b.jpg")).toBe("https://b1.cdn.zanao.com/upload/a/b.jpg@!sm_w400");
    expect(skbirdImageUrl("/upload/a/b.jpg")).toBe("https://b1.cdn.zanao.com/upload/a/b.jpg@!sm_w400");
  });
...
describe("SkbirdClient 请求契约", () => {
  ...
  it("thread()：返回详情与评论签名（data.t_sign）", async () => {
    sendMock.mockResolvedValue(
      okResponse({ detail: { thread_id: 123, title: "t" }, t_sign: "SIGN+BLOB==" }),
    );
  ...
  it("categories()：解析 cate_list", async () => {
    sendMock.mockResolvedValue(
      okResponse({
        cate_list: [{ cate_id: "101", name: "二手闲置", summary: "本校线下交易", icon_path: "upload/x.png" }],
      }),
    );
    const list = await new SkbirdClient(CONFIG).categories();
    expect(list).toEqual([{ cateId: "101", name: "二手闲置", summary: "本校线下交易", iconPath: "upload/x.png" }]);
  });
  ...
  it("messages()/unreadNum()/likeStat()/userStats()：字段映射", async () => {
    ...
    sendMock.mockResolvedValueOnce(
      okResponse({ list: [{ msg_id: "1", content: "c", nickname: "n", p_time: 1787378674 }] }),
    );
    const msgs = await client.messages();
    expect(msgs[0]).toMatchObject({ id: "1", content: "c", nickname: "n", time: 1787378674 });
    ...
    await expect(client.unreadNum()).resolves.toEqual({ imCount: 1, count: 2, markCount: 3 });

    sendMock.mockResolvedValueOnce(okResponse({ chat_show: false, like_num: "9", new_like_num: 4 }));
    await expect(client.likeStat()).resolves.toEqual({ likeNum: 9, newLikeNum: 4 });

    sendMock.mockResolvedValueOnce(okResponse({ thread_count: 7, user_coin: 42 }));
    await expect(client.userStats()).resolves.toEqual({ threadCount: 7, userCoin: 42 });
  });
  ...
  it("评论映射：reply_list 递归为 replies", async () => {
    ...
    expect(list[0]!.replies[0]).toMatchObject({ commentId: "2", content: "sub", likeCount: 3 });
  });

  it("unlockThread()：帖在热榜时直接取全文+图片，不走 mark 链", async () => {
    sendMock.mockResolvedValueOnce(
      okResponse({
        list: [{ thread_id: "9", title: "认证帖标题", content: "全文", img_paths: ["upload/x.jpg"], cert_show: "10" }],
      }),
    );
    const t = await new SkbirdClient(CONFIG).unlockThread("9");
    expect(t).toMatchObject({ threadId: "9", title: "认证帖标题", imgPaths: ["upload/x.jpg"] });
  ...
  it("unlockThread()：热榜/悬赏都未命中时走 mark→mark/list→unmark（纯文字）", async () => {
    sendMock
      ...
      .mockResolvedValueOnce(
        okResponse({ list: [{ thread_id: "9", title: "全文标题", content: "全文" }] }),
      ) // mark/list
  ...
  it("unlockThread()：帖在悬赏流时取全文+图片，不走 mark 链", async () => {
    sendMock
      ...
      .mockResolvedValueOnce(
        okResponse({
          list: [{ thread_id: "9", title: "悬赏认证帖", content: "全文", img_paths: ["upload/r.jpg"] }],
        }),
      ); // rewardlist 命中
    const t = await new SkbirdClient(CONFIG).unlockThread("9");
