...
function GachaCard({
  ...
  return (
    ...
    >
      <div
        className={cn(
          "relative h-56 transition-transform duration-700 [transform-style:preserve-3d]",
      ...
      >
        ...
        >
          ...
          <span className="text-3xl font-bold tabular-nums">{scoreText}</span>
...
export function GradeGachaModal({
  ...
  const allFlipped = pending.every((p) => flipped.has(p.key));
  ...
  return (
    <div
      ...
      className="animate-in fade-in-0 fixed inset-0 z-[70] flex flex-col bg-background/95 backdrop-blur-md duration-300"
    >
      ...
      <footer className="flex items-center justify-center gap-3 px-4 pt-2 pb-[max(1rem,var(--safe-area-inset-bottom,env(safe-area-inset-bottom,0px)))]">
        {allFlipped ? (
